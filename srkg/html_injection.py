...
def inject_controls(
    ...
    <style>
      ...
      .kg-canvas-select:hover,
      .kg-canvas-select:focus {
        background: #eef3fd;
        outline: none;
      }
      .kg-canvas-select {
    ...
    #info_panel .optional-detail-body {
        border-left: 2px solid #c8d7f2;
        margin-top: 0.45em;
        padding-left: 0.75em;
    }

    #info_panel .concept-section {
        margin: 0;
    }

    #info_panel .concept-line {
        min-height: 1.45em;
    }

    #info_panel .kg-add-note-row {
        display: none;
        line-height: 1;
        margin: 0.08em 0;
    }

    #info_panel.kg-note-editing .kg-add-note-row {
        display: block;
    }

    #info_panel .kg-add-note {
        background: #f6f1dc;
        border: 1px dashed #b08b16;
        border-radius: 3px;
        color: #6b5200;
        cursor: pointer;
        font: inherit;
    ...
    #info_panel .kg-add-note:hover,
    #info_panel .kg-add-note:focus {
        background: #fff5bf;
        opacity: 1;
        outline: none;
    }

    #info_panel .user-note {
        margin: 0.75em 0;
        padding: 0;
    }

    #info_panel .user-note > summary {
        color: #7a5800;
        cursor: pointer;
        font-weight: 700;
    }
    ...
    #info_panel .user-note-editor label {
        color: #444;
        display: flex;
        flex-direction: column;
        font-size: 0.9em;
    ...
    #info_panel .user-note-editor input,
    ...
    div.vis-tooltip .kg-tooltip-title,
    #kg_node_tooltip .kg-tooltip-title {
        font-weight: 700;
        margin-bottom: 0.45em;
    }

    div.vis-tooltip .kg-tooltip-definition,
    #kg_node_tooltip .kg-tooltip-definition {
        margin-bottom: 0.45em;
    }

    div.vis-tooltip .kg-tooltip-section-title,
    #kg_node_tooltip .kg-tooltip-section-title {
        font-weight: 700;
        margin-top: 0.45em;
    }
    ...
    body.kg-graph-hidden #mynetwork,
    body.kg-graph-hidden #kg_node_labels,
    body.kg-graph-hidden #kg_view_title {
        opacity: 0;
        pointer-events: none;
    }
    ...
    @media (max-width: 850px) {
        ...
        #kg_graph_view_select {
            left: calc(max(8px, env(safe-area-inset-left)) + 222px);
            min-width: 154px;
            max-width: calc(100vw - 230px - env(safe-area-inset-left) - env(safe-area-inset-right));
        }
        ...
        body.kg-graph-hidden #info_panel {
    ...
    <select id="kg_graph_view_select" class="kg-canvas-select" aria-label="Graph view">
      <option value="hide">Hide graph</option>
      <option value="all" selected>All</option>
      <option value="neighbourhood-1">Neighbourhood (1)</option>
      <option value="neighbourhood-2">Neighbourhood (2)</option>
      <option value="descendants">Descendants</option>
    </select>
    <div id="kg_controls">
      <b>Knowledge graph explorer</b><br>
      <button onclick="kgShowEdgeKey()">Edge key</button>
      <button id="kg_features_button" type="button" onclick="kgShowSplash()">Features</button>
      ...
      <details id="kg_notes_section" class="kg-fold">
        <summary>Notes</summary>
        <label class="kg-edge-filter">
          <input id="kg_notes_edit_toggle" type="checkbox">
          <span class="kg-edge-filter-label">Note editing</span>
        </label>
        <button id="kg_notes_export" type="button" onclick="kgExportNotes()">Export notes</button>
        <label>
          <button id="kg_notes_import_button" type="button">Import notes</button>
    ...
    <dialog id="kg_splash_dialog" class="kg-splash" aria-labelledby="kg_splash_title">
      <h2 id="kg_splash_title">Knowledge graph browser</h2>
      <p>Use the graph and details panel together while studying each concept.</p>
      <ul>
        <li>Click graph nodes or the searchable concept list to open concept details.</li>
        <li>Switch graph views between all concepts, one-hop/two-hop neighbourhoods, descendants, or hidden graph.</li>
        <li>Filter edge types and inspect relation meanings from the edge key.</li>
        <li>Open optional derivation details inline. <span class="kg-new-badge">NEW</span></li>
        <li>Add personal notes inline, then export or import them as CSV. <span class="kg-new-badge">NEW</span></li>
        <li>Zoom the details text independently from the graph. <span class="kg-new-badge">NEW</span></li>
      </ul>
      <div class="kg-splash-actions">
        <button id="kg_splash_dismiss" type="button">Dismiss</button>
      </div>
    </dialog>
    ...
    <script type="text/javascript">
      ...
      function kgAfterReady() {
        ...
        var userNotesStorageKey = "srkg.userNotes.v1";
        var noteEditingStorageKey = "srkg.noteEditing.v1";
        var splashDismissedStorageKey = "srkg.splash.dismissed.v1";
        ...
        function showNodeTooltip(nodeId, pointer) {
          if (!nodeTooltip || currentView.mode === "hide" || !getConcept(nodeId)) { return; }
        ...
        function graphViewSelectValue() {
          if (currentView.mode === "hide") { return "hide"; }
          if (currentView.mode === "neighbourhood") {
            return "neighbourhood-" + String(currentView.radius || 1);
          }
          if (currentView.mode === "descendants") { return "descendants"; }
          return "all";
        }
        ...
        function updateGraphViewControls() {
          ...
          if (select) {
            select.value = graphViewSelectValue();
          }
          document.body.classList.toggle("kg-graph-hidden", currentView.mode === "hide");
        }
        ...
        function optionalDetailTitlesForConcept(concept) {
          return []
            .concat(optionalDetailTitlesFromText(concept.definition_new || ""))
            .concat(optionalDetailTitlesFromText(concept.derivation_new || ""))
            .concat(optionalDetailTitlesFromText(concept.explanation_new || ""));
        }
        ...
        function conceptTooltipHtml(nodeId) {
          ...
          var definition = concept.definition_new || "";
        ...
        function applyCurrentView() {
          ...
          if (currentView.mode === "neighbourhood" && currentView.nodeId !== null) {
            kgApplyNeighbourhood(currentView.nodeId, true, currentView.radius || 1);
            return;
          }
        ...
        function showConcept(nodeId, options) {
          ...
          [
            ...
            html += renderConceptSection(nodeId, section[0], section[1]);
          });
        ...
        function focusNeighbourhood(nodeId, statusPrefix, options) {
          options = options || {};
          if (!getConcept(nodeId)) { return; }
          var radius = Math.max(1, Math.min(2, Math.floor(Number(options.radius || currentView.radius || 1))));
          kgApplyNeighbourhood(nodeId, true, radius);
          showConcept(nodeId);
          setActiveConceptItem(nodeId);
          if (!options.skipHistory) {
            pushConceptHistory(nodeId, "neighbourhood-" + radius);
          }
        ...
        window.kgSetGraphView = function(value) {
          ...
          if (value === "hide") {
            ...
            kgReset();
          } else if (value === "neighbourhood-1" || value === "neighbourhood-2") {
            if (!nodeId) {
              document.getElementById("kg_status").innerText = "Select a node first.";
              updateGraphViewControls();
              return;
            }
            focusNeighbourhood(nodeId, null, {radius: value === "neighbourhood-2" ? 2 : 1});
        ...
        function focusHiddenConcept(nodeId, statusPrefix, options) {
          ...
          if (!getConcept(nodeId)) { return; }
          activeNodeId = nodeId;
          currentView = {mode: "hide", nodeId: nodeId};
          kgHideGraph(true);
          showConcept(nodeId, {
            searchQuery: options.searchQuery,
            scrollToSearchMatch: options.scrollToSearchMatch
          ...
          setActiveConceptItem(nodeId);
          if (!options.skipHistory) {
            pushConceptHistory(nodeId, "hide");
          }
          if (statusPrefix) {
            document.getElementById("kg_status").innerText =
        ...
        function kgApplyNeighbourhood(nodeId, preserveStatus, radius) {
          restoreHoveredEdge();
          activeNodeId = nodeId;
          radius = Math.max(1, Math.min(2, Math.floor(Number(radius) || 1)));
          currentView = {mode: "neighbourhood", nodeId: nodeId, radius: radius};
          updateGraphViewControls();
        ...
        window.kgFocusSelected = function() {
          ...
          focusNeighbourhood(nodeId, null, {radius: currentView.radius || 1});
        };
        ...
        window.addEventListener("popstate", function(event) {
          ...
          if (nodeId && getConcept(nodeId)) {
            ...
            } else if (mode === "neighbourhood" || mode === "neighbourhood-1") {
              focusNeighbourhood(nodeId, null, {skipHistory: true, radius: 1});
            } else if (mode === "neighbourhood-2") {
              focusNeighbourhood(nodeId, null, {skipHistory: true, radius: 2});
        ...
        document.getElementById("info_panel").addEventListener("click", function(e) {
          ...
          if (currentView.mode === "neighbourhood") {
            focusNeighbourhood(id, "Selected", {radius: currentView.radius || 1});
          } else if (currentView.mode === "hide") {
